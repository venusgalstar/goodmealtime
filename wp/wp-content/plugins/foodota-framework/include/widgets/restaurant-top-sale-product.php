 ...
 */
class Food_Top_rated_Product_Widget extends WP_Widget
{
    /**
    ...
    public function __construct()
    {
        $widget_options = array(
            ...
            'description' => __('A Restaurant Top Rated Product Widget', 'foodota-framework'),
        );
        parent::__construct('food_top_rated_product_widget', 'Restaurant:Top Rated Products', $widget_options);
    }
    ...
    public function widget($args, $instance)
    {
        ...
        echo __('', 'foodota-framework');
         ...
         <div class="top-sale-product owl-carousel owl-theme">
             ...
             <div class="item">
                 <div class="res-3-box res-fl-product">
                     <div class="res-2-img"> <a href="<?php echo esc_url($store_url); ?>">
                             <img src="<?php echo esc_url($product_image); ?>" alt="<?php echo esc_attr(get_post_meta($food_child_more['product-image-id'], '_wp_attachment_image_alt', TRUE)); ?>" class="img-fluid">
                         </a>
                         <div class="res-3-icons fav-check"><?php echo ($shipment_status); ?></div>
                         <div class="res-3-logo-img"> <a href="<?php echo esc_url($store_url); ?>">
                                 <img src="<?php echo esc_url($gravatar); ?>" alt="<?php echo esc_attr(get_post_meta($gravatar_id, '_wp_attachment_image_alt', TRUE)); ?>" class="img-fluid">
                             </a>
                         </div>
                     ...
                     <div class="res-2-bg-white">
                         <div class="res-2-inner">
                             ...
                             <div class="res-2-map-product">
                                 <img src="<?php echo esc_url(trailingslashit(get_template_directory_uri()).'libs/images/location.png');?>" alt="<?php echo esc_attr(get_post_meta($image_alt_id, '_wp_attachment_image_alt', TRUE)); ?>" class="img-fluid">
                         ...
                         <div class="res-fl-main-cat-content-3 slider-btn">
                             <?php if ($food_child_more['product_type'] === "variable" || $food_child_more['product_type'] === "grouped") {?>
                                 <button type="button" class="order-btn btn btn-theme cart-check-btn button product_type_simple add_to_cart_button ajax_add_to_cart product-quantity-btn openNav"> <?php echo esc_html('Order Now', 'foodota') ?></button>
                             <?php } else { ?>
                                     ...
                                     <button type="button" data-quantity="1"
                                             ...
                                             rel="nofollow"><?php echo esc_html('Order Now', 'foodota-framework') ?></button>
    ...
    public function form($instance)
    {
        if (isset($instance['title'])) {
            ...
            $title = __('Best Selling Products', 'foodota-framework');
        }
        ?>
        <p>
...
if (in_array('wc-multivendor-marketplace/wc-multivendor-marketplace.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    add_action('widgets_init', 'register_top_product_widgets');
}

if (!function_exists('register_top_product_widgets')) {
    function register_top_product_widgets()
    {
        register_widget('Food_Top_rated_Product_Widget');
    }
}